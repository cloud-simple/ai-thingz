...
|---|---|
| `0.3.1` | `--mr` and `--merge` on GitLab open the merge request on the fork again: `glab mr create` is given the fork as `--head` as well as `--repo`, because glab creates the request on the project the source branch lives in and, without `--head`, works that project out from the remotes - which answered with `upstream`, so the request was posted to the original project and refused with a 403. `glab mr merge` finds its request through `--repo` alone and is unchanged |
