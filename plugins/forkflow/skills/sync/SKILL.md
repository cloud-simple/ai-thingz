...
5. **Merge request.** The script prints the command; run it in the same invocation with `--mr`
   ...
   carries `--repo <the fork's URL>`, and on GitLab `--head <the fork's URL>` too: hand it over
   exactly as printed, never trimmed - without them the merge request is opened on the original
   project (`rules.md`). The
