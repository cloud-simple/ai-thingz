...
- [x] `rules.md`: the six hard rules (incl. the ship-MR merge button and the GitHub SHA-rewrite note) and the sequence rule, the layout diagram, plus the platform note (a stale sync MR is redone with `sync`, never "Rebase"d in the UI) - short, quotable, no project names
- [x] `status/SKILL.md`: frontmatter (name, description with triggers "forkflow status", "where are we vs upstream", "how far behind upstream", `allowed-tools: Bash, Read`); run `${CLAUDE_PLUGIN_ROOT}/scripts/forkflow.py status` (`--fetch` when the user asks for current numbers), explain the numbers and the layout, if the setup line shows LIVE/missing or the trunk is missing say so and offer `/forkflow:setup`
- [x] `sync/SKILL.md`: triggers "sync upstream", "pull in upstream", "forkflow sync"; read `rules.md`; dry run -> real run -> exit 4: resolve conflicts file by file showing both sides, `git add`, `--continue` -> read the both-sides table, look at `CHECK` rows -> MR with `--mr` -> report mirror advance, branch, merge commit, backup + rollback line, MR URL, what was not done; never merge the MR, never touch the trunk; closing step: how the local trunk catches up after the MR merges
- [x] `ship/SKILL.md`: triggers "ship this branch", "land this branch", "forkflow ship", "squash and open the MR"; same shape; WARNING list of upstream-tracked files shown, ask only if the branch adds to that list unexpectedly; rebase conflicts -> resolve, `git rebase --continue`, `ship --continue`; closing step incl. deleting the local branch after a GitHub "Rebase and merge"
- [x] `setup/SKILL.md`: triggers "forkflow setup", "set up the fork", "protect the trunk locally"; fresh clone -> ask for the upstream URL and pass `--upstream-url`; run, then walk through the default-branch/merge-method/protection report and the printed fix commands; explain what the hook refuses and that it validates against the last fetch; a diverged `main` -> point to the README adoption recipe and stop (never reset it)
- [x] every SKILL.md: state up front that the script never pushes or rebases the trunk, never commits on the mirror, and that Claude must not either; `allowed-tools: Bash, Read, AskUserQuestion` for sync/ship/setup; no project-specific names anywhere
- [x] verify frontmatter parses (`python3 -c "import yaml"` if available, else eyeball) and every script path uses `${CLAUDE_PLUGIN_ROOT}/scripts/forkflow.py`
- [x] run `--test` - must still pass before task 13
- ➕ [x] PyYAML is not installed on this machine, so instead of eyeballing the frontmatter a
  throwaway checker parsed it as a flat `key: value` block and decoded every double-quoted value
  with `json.loads` (the escape sets overlap), asserting the three expected keys, `name` ==
  directory name, and `allowed-tools` per skill

### Task 13: Verify acceptance criteria
- [x] verify all requirements from Overview are implemented: six rules mechanically enforced (script and hook), sync/ship/status/setup behave as specified, exit codes as tabled, `--dry-run` creates no branches/commits/pushes/config/hooks/mirror moves and still previews the pending merge
- [x] grep the script as a backstop: every push goes through `push()`, `push_mirror()` or `bootstrap_trunk()`; no `rebase` whose target can be the trunk; no `--force` (only `--force-with-lease=`), no `--no-verify`; `update-ref`/`merge --ff-only` only inside `advance_mirror`; no project-specific names in `plugins/forkflow/` or README
- [x] verify edge cases: git < 2.38 note, detached HEAD, `--continue` misuse (both subcommands), foreign hook, `core.hooksPath`, unknown platform, config without `tomllib`, single-branch clone, mirror checked out here / in a linked worktree, fresh fork, diverged mirror, unfetched upstream
- [x] run full test suite: `python3 plugins/forkflow/scripts/forkflow.py --test` and `python3 -m unittest discover -s tests` (nocomment untouched)
- [x] load the plugin locally with `claude --plugin-dir plugins/forkflow` (falling back to `/plugin marketplace add <path to this checkout>` + `/plugin install forkflow@ai-thingz` if the flag is unavailable) and check that `/forkflow:status` resolves as a skill and the script path expands; if the `/plugin:skill` form does not resolve, fall back to description-only triggers and note it in the README - **interactive `claude` session (skipped - not automatable)**; everything else it would prove was checked non-interactively: `marketplace.json` and both `plugin.json` files parse and every `source` resolves to a directory holding a `.claude-plugin/plugin.json`; all four `SKILL.md` frontmatters parse with `name` == directory name and the `allowed-tools` the plan asks for; every `${CLAUDE_PLUGIN_ROOT}` path in them (`/scripts/forkflow.py`, `/references/rules.md`) resolves to an existing file and no SKILL.md names the script by any other path; the script runs with `CLAUDE_PLUGIN_ROOT` set to the plugin directory
- [x] run `/forkflow:status` on this repo (no upstream -> the exit 2 hint) to confirm the end-to-end wiring - `python3 plugins/forkflow/scripts/forkflow.py status` exits 2 with "no remote for the original project; run `forkflow setup --upstream-url <URL>` ..." (the skill body is that one command)
- ➕ [x] the grep backstop became a permanent test class, `TestSourceInvariants`: it parses the
  code above `run_tests()` with `ast`, maps every line to its innermost function, and asserts the
  argv token `"push"` appears only in the three push helpers, `"update-ref"` and
  `"merge", "--ff-only"` only in `advance_mirror`, `"rebase"` only in `rebase_onto`,
  `--force-with-lease` only in `push()`, and that `--no-verify` and a bare `"--force"` git token
  appear nowhere (plus: `subprocess` is reached only through `git`/`git_ok`/`git_rc`/`shell`/
  `open_mr`/`api_get`). A one-off grep would have gone stale the first time the script changed
- ➕ [x] exit 130 was the one tabled code with no test; `TestMainWiring.test_an_interrupt_is_exit_130`
